...
public class CaseFileService : ICaseFileService
{
    ...
    public async Task BindFilesToTaskAsync(Guid caseId, string taskNo, Guid taskId)
    {
        ...
        await RebuildProjectRawDataAsync(task.Case.RecordId);
    ...
    public async Task DeleteFilesByTaskIdAsync(Guid taskId)
    {
        ...
        var recordId = files.FirstOrDefault()?.RecordId;
        ...
        if (recordId.HasValue)
            await RebuildProjectRawDataAsync(recordId.Value);
    ...
    private async Task RebuildProjectRawDataAsync(Guid recordId)
    {
